...
So `this.name` means "the `name` field on this current object."
...
* The 8 primitive types get 'default' values:
...
That particular crash is called a `NullPointerException`, and it is one of the most common ways that 
Java programs can crash. Sometimes `null` values are unavoidable, but it is good practice for now to make sure
you always set variables and fields so they are not `null`.
